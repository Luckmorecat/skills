...
Create it outside the repository, which puts a commit of the plan out of reach rather than merely against the rules:
...
mkdir -p "/tmp/plans/$repo/$(date +%F)-<slug>"
```
...
```
/tmp/plans/<repo>/2026-08-21-<slug>/
  spine.md
  map.md
  01-<slug>.md
  02-<slug>.md
...
- **Intent source** — the user request and any approved `$shape-project` brief.
...
- **Verify** — the command that proves this slice green.
